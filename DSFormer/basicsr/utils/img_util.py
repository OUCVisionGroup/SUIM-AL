...
#     return img_lq, img_gt
#双分支
def padding(img_lq1, img_lq2, img_gt, gt_size):
    h, w, _ = img_lq1.shape
    ...
    img_lq1 = cv2.copyMakeBorder(img_lq1, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_lq2 = cv2.copyMakeBorder(img_lq2, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt = cv2.copyMakeBorder(img_gt, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    ...
    return img_lq1, img_lq2, img_gt
#双分支
def padding_DP(img_lq1, img_lq2, img_gt, gt_size):
    h, w, _ = img_lq1.shape
...
# def padding_DP(img, divisor=4):
#     """Dummy padding for DP (Deep Privacy) compatibility.
#     If the original repo does not need special logic,
#     just use the same as `padding`."""
#     return padding(img, divisor)
#
# def imfrombytesDP(img_bytes):
#     """Dummy wrapper; fallback to normal imfrombytes."""

...
class Dataset_PairedImage(data.Dataset):
    ...
    现在支持 **三个输入**：
    - lq1: 低质量输入1（如：普通模糊/降噪图）
    - lq2: 低质量输入2（如：深度图/额外辅助图）
    - gt: 目标高质量图
    """
    ...
    def __init__(self, opt):
        ...
        # **🔥 这里变成3个路径！**
        self.gt_folder, self.lq1_folder, self.lq2_folder = opt['dataroot_gt'], opt['dataroot_lq1'], opt['dataroot_lq2']

        self.filename_tmpl = opt.get('filename_tmpl', '{}')

        if self.io_backend_opt['type'] == 'lmdb':
            self.io_backend_opt['db_paths'] = [self.lq1_folder, self.lq2_folder, self.gt_folder]
            self.io_backend_opt['client_keys'] = ['lq1', 'lq2', 'gt']
            self.paths = paired_paths_from_lmdb(
                [self.lq1_folder, self.lq2_folder, self.gt_folder], ['lq1', 'lq2', 'gt'])
        elif 'meta_info_file' in opt and opt['meta_info_file'] is not None:
            self.paths = paired_paths_from_meta_info_file(
                [self.lq1_folder, self.lq2_folder, self.gt_folder],
                ['lq1', 'lq2', 'gt'],
                opt['meta_info_file'],
                self.filename_tmpl
            )
    ...
    def __getitem__(self, index):
        ...
        lq1_path = self.paths[index]['lq1_path']
        lq2_path = self.paths[index]['lq2_path']

        img_bytes = self.file_client.get(gt_path, 'gt')
        img_gt = imfrombytes(img_bytes, float32=True)

        img_bytes = self.file_client.get(lq1_path, 'lq1')
        ...
        if self.opt['phase'] == 'train':
            ...
            # **🔥 padding**
            img_lq1, img_lq2, img_gt = padding(img_lq1, img_lq2, img_gt, gt_size)

            # **🔥 随机裁剪**
            img_gt, img_lq1, img_lq2 = paired_random_crop_DP(img_gt, img_lq1, img_lq2, gt_size, scale, gt_path)

            # **🔥 随机翻转/旋转**
            if self.geometric_augs:
                img_gt, img_lq1, img_lq2 = random_augmentation(img_gt, img_lq1, img_lq2)

        # **🔥 转换：BGR → RGB，HWC → CHW，numpy → tensor**
        img_gt, img_lq1, img_lq2 = img2tensor([img_gt, img_lq1, img_lq2], bgr2rgb=True, float32=True)

        # **🔥 归一化**
        if self.mean is not None or self.std is not None:
            normalize(img_lq1, self.mean, self.std, inplace=True)

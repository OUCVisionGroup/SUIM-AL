...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    """从多个文件夹生成配对的路径，支持 lq1, lq2, gt 三个文件夹！"""

    assert len(folders) == 3, (
        'The len of folders should be 3 with [lq1_folder, lq2_folder, gt_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 3 with [lq1_key, lq2_key, gt_key]. '
        f'But got {len(keys)}')

    lq1_folder, lq2_folder, gt_folder = folders
    lq1_key, lq2_key, gt_key = keys

    lq1_paths = list(scandir(lq1_folder))
    lq2_paths = list(scandir(lq2_folder))
    gt_paths = list(scandir(gt_folder))

    # **确保所有文件数量一致**
    assert len(lq1_paths) == len(lq2_paths) == len(gt_paths), (
        f'{lq1_key}, {lq2_key} 和 {gt_key} 数据集的图像数量不一致: '
        f'{len(lq1_paths)}, {len(lq2_paths)}, {len(gt_paths)}.')
    ...
    for idx in range(len(gt_paths)):
        ...
        lq1_path = osp.join(lq1_folder, f'{filename_tmpl.format(basename)}{ext}')
        lq2_path = osp.join(lq2_folder, f'{filename_tmpl.format(basename)}{ext}')
        gt_path = osp.join(gt_folder, gt_path)
        # print(f'✅ lq1_path: {lq1_path}')
        # print(f'✅ lq1_paths: {lq1_paths}')

        assert osp.basename(lq1_path) in lq1_paths, f'{lq1_path} 不在 {lq1_key} 文件夹中！'
        assert osp.basename(lq2_path) in lq2_paths, f'{lq2_path} 不在 {lq2_key} 文件夹中！'

        paths.append(
            {
                f'{lq1_key}_path': lq1_path,
                f'{lq2_key}_path': lq2_path,
                f'{gt_key}_path': gt_path
            }
        )

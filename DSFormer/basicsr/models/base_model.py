...
#共享权重
    ...
    #
    #     # scheduler_type = train_opt['scheduler'].pop('type')  # 这里直接 pop()，删掉了 type，导致后面找不到
    #
    #     scheduler_type = train_opt['scheduler'].get('type', '默认调度器')  # 不再 pop，防止删掉 type
    #
    ...
    def setup_schedulers(self):
        ...
        if 'scheduler' not in train_opt:
            raise ValueError("❌ `scheduler` 配置没找到！检查你的 YAML 配置！")

        scheduler_type = train_opt['scheduler'].get('type', 'CosineAnnealingLR')  # **✅ 不要 pop()，防止删掉 type**
        scheduler_opts = {k: v for k, v in train_opt['scheduler'].items() if k != 'type'}  # **🔥 过滤掉 'type'**

        if not self.optimizers:
            raise ValueError("❌ 没有找到 `self.optimizers`，你 optimizer 还没初始化吧？")

        for optimizer in self.optimizers:
            if scheduler_type == 'MultiStepLR':
                self.schedulers.append(lr_scheduler.MultiStepLR(optimizer, **scheduler_opts))
            elif scheduler_type == 'MultiStepRestartLR':
                self.schedulers.append(lr_scheduler.MultiStepRestartLR(optimizer, **scheduler_opts))
            elif scheduler_type == 'CosineAnnealingRestartLR':
                self.schedulers.append(lr_scheduler.CosineAnnealingRestartLR(optimizer, **scheduler_opts))
            elif scheduler_type == 'CosineAnnealingWarmupRestarts':
                self.schedulers.append(lr_scheduler.CosineAnnealingWarmupRestarts(optimizer, **scheduler_opts))
            elif scheduler_type == 'CosineAnnealingRestartCyclicLR':
                self.schedulers.append(lr_scheduler.CosineAnnealingRestartCyclicLR(optimizer, **scheduler_opts))
            elif scheduler_type == 'TrueCosineAnnealingLR':
                print('..', 'cosineannealingLR')
                self.schedulers.append(torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, **scheduler_opts))
            elif scheduler_type == 'CosineAnnealingLRWithRestart':
                print('..', 'CosineAnnealingLR_With_Restart')
                self.schedulers.append(lr_scheduler.CosineAnnealingLRWithRestart(optimizer, **scheduler_opts))
            elif scheduler_type == 'LinearLR':
                self.schedulers.append(lr_scheduler.LinearLR(optimizer, **scheduler_opts))
            elif scheduler_type == 'VibrateLR':
                self.schedulers.append(lr_scheduler.VibrateLR(optimizer, **scheduler_opts))
            else:
                raise NotImplementedError(f'❌ 这个 `{scheduler_type}` 学习率调度器不支持！')

        # logger = get_root_logger()
        logger.info(f'✅ 学习率调度器 `{scheduler_type}` 设置成功！')
    ...
    def save_training_state(self, epoch, current_iter, **kwargs):
        ...
        if current_iter != -1:
            ...
            state['best_metric'] = kwargs.get('best_metric', None)  # **✅ 没有 best_metric 就设成 None**
        ...
        with torch.no_grad():
            ...
            for name, value in loss_dict.items():
                ...
                if isinstance(value, torch.Tensor):
                    log_dict[name] = value.mean().item()  # 对于Tensor类型，取平均值
                else:
                    log_dict[name] = value  # 对于非Tensor类型（如float），直接保存值

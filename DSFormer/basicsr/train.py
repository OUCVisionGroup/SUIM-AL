...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        '--opt', type=str, default='/data/gez/Project/DSFormer.yml',
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        if phase == 'train':
            ...
            train_sampler = EnlargedSampler(train_set, opt['world_size'],
                                            opt['rank'],
                                            dataset_enlarge_ratio)  # 缺少关键字 world_size 和 rank，train_sampler是做什么？从get_dist_info得到
            # stx()
            ...
            total_epochs = math.ceil(
                total_iters / (num_iter_per_epoch))  # 一个iteration就是一次 inference + backward，总的iteration是不变的
            logger.info(
                'Training statistics:'
...
class RealImageDataset(Dataset):
    def __init__(self, folder, fixed_size=256):
        self.folder = folder
        self.files = [
...
def get_real_image_dataloader(folder, batch_size, fixed_size=256, num_workers=4):
    dataset = RealImageDataset(folder, fixed_size)
...
real_image_folder = "/data/gez/datasets/NUID/222"
batch_size = 1  # 你想控制的 batch 大小

real_dataloader = get_real_image_dataloader(real_image_folder, batch_size)
...
for real_imgs in real_dataloader:
    ...
    # depth_model = DepthModel(encoder='vits', features=64, out_channels=[48, 96, 192, 384]).to(device)
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            # 取出两组输入
            lq1 = train_data['lq1'].to(model.device)
            lq2 = train_data['lq2'].to(model.device)
            gt = train_data['gt'].to(model.device)

            for path in train_data['lq1_path']:
                image_name = os.path.splitext(os.path.basename(path))[0] + ".png"

            for path in train_data['lq2_path']:
                image_name = os.path.splitext(os.path.basename(path))[0] + ".png"

            # 转成 Tensor 并拼接
            # **喂入数据**
            saliency = real_imgs.to(device)
            model.feed_train_data(lq1,lq2,gt)  # 这里要确保 `feed_train_data` 支持多个输入
            model.optimize_parameters(current_iter)  # 优化共享权重的模型

            # **批量采样**
            if mini_batch_size < batch_size:  # 默认 batch_size 组图片，小于的话就要随机采样
